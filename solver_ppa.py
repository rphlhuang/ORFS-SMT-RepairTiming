...
class SMTsolver:
    def __init__(self, data):
        ...
        # --- AREA MINIMIZATION ---
        total_area = Real("total_area")
        area_terms = []
        for stage in data['path_data']['stages']:
            slot_id = stage['slot_id']
            for choice in stage['choices']:
                cell_name = choice['cell_type']
                area_val = choice['area']
                z3_var = self.decision_vars[(slot_id, cell_name)]
                area_terms.append(If(z3_var, area_val, 0))
        self.solver.add(total_area == Sum(area_terms))
        ...
        # Optimize area
        self.solver.minimize(total_area)
